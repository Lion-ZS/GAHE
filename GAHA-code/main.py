...
class Experiment:
    def __init__(self, num_iterations, batch_size, lr, dr, rdim, edim, m, K, max_ary,cu,rate):
    ...
    def train_and_eval(self):
        ...
        model = GAHE(self.K, len(d.rel2id), len(d.ent2id), self.rdim, self.edim, self.m, self.max_ary,self.cu,self.rate, device, **self.kwargs)
        model.to(device)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--c", type=float, default=0.01, nargs="?", help="c rate.")
    ...
    parser.add_argument("--rate", type=float, default=0.2, nargs="?", help="rate.")
    ...
    Exp = Experiment(num_iterations=param['num_iterations'], batch_size=param['batch_size'], lr=param['lr'], dr=param['dr'],
                     rdim=param['rdim'], edim=param['edim'], m=param['m'], K=param['K'], max_ary=d.max_ary,cu=param['cu'],rate=param['rate'])
    Exp.train_and_eval()