...
class GAHE(nn.Module):
    def __init__(self, K, n_r, n_v, rdim, vdim, n_parts, max_ary, device, **kwargs):
        ...
        self.c = torch.nn.Parameter(torch.tensor(1.0).to(device), requires_grad=True)
        self.u = torch.nn.Parameter(torch.tensor(-1.0).to(device), requires_grad=True)
    ...
    def forward(self, rel_idx, value_idx, miss_value_domain):
        ...
        for index in range(V0.shape[2]):
            ...
            h_H = self.h1_interaction(h, q, c=0.1) 
            h_S = self.h2_interaction(h, q, u=-0.1) 
            h_multi_space = self.multi_space_fusion(h_E, h_H, h_S)
            score += torch.einsum('bak,bnak,bak->bn', self.givens_trans(RelV_bi[:,:,:,index],h_multi_space), V0_miss,role)   
        return score
